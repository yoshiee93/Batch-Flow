...
The Settings page (`client/src/features/catalog/pages/Settings.tsx`) is organized into shadcn Tabs (`general`, `production`, `labels`, `data`, `security`). Active tab is driven by the `?tab=` query string. The Label Templates admin (formerly at `/labels`) lives inside the **Labels** tab — `LabelTemplatesPanel` is the default export of `client/src/features/labels/pages/Labels.tsx`. The standalone `/labels` route now redirects to `/settings?tab=labels` and the Labels item has been removed from the sidebar. The Labels tab also contains placeholder cards for a future Custom Label Builder and Print Custom Label, and the Security tab is a placeholder for upcoming user/role/audit settings.
