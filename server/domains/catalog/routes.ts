...
// Writes and Settings-page access require settings.view.
const adminOnly = requirePermission("settings.view");
// Reads are also available to production and inventory roles so they can
// select products/materials/categories/recipes in their respective workflows
// without needing full Settings access.
const canReadCatalog = requireAnyPermission(["settings.view", "production.view", "inventory.view"]);

export const catalogRouter = Router();

catalogRouter.get("/categories", canReadCatalog, asyncHandler(async (_req, res) => {
  res.json(await svc.getCategories());
}));

catalogRouter.get("/categories/:id", canReadCatalog, asyncHandler(async (req, res) => {
  const category = await svc.getCategory(req.params.id);
  if (!category) return res.status(404).json({ error: "Category not found" });
  res.json(category);
}));

catalogRouter.post("/categories", adminOnly, asyncHandler(async (req, res) => {
  const data = insertCategorySchema.parse(req.body);
  res.status(201).json(await svc.createCategory(data));
}));

catalogRouter.patch("/categories/:id", adminOnly, asyncHandler(async (req, res) => {
...
catalogRouter.delete("/categories/:id", adminOnly, asyncHandler(async (req, res) => {
  try {
...
catalogRouter.get("/products/by-category/:categoryId", canReadCatalog, asyncHandler(async (req, res) => {
  res.json(await svc.getProductsByCategory(req.params.categoryId));
}));

catalogRouter.get("/products/:id/recipes", canReadCatalog, asyncHandler(async (req, res) => {
  res.json(await svc.getRecipesByProduct(req.params.id));
}));

catalogRouter.get("/products/:id", canReadCatalog, asyncHandler(async (req, res) => {
  const product = await svc.getProduct(req.params.id);
  if (!product) return res.status(404).json({ error: "Product not found" });
  res.json(product);
}));

catalogRouter.get("/products", canReadCatalog, asyncHandler(async (_req, res) => {
  res.json(await svc.getProducts());
}));

catalogRouter.post("/products", adminOnly, asyncHandler(async (req, res) => {
  const data = insertProductSchema.parse(req.body);
  res.status(201).json(await svc.createProduct(data));
}));

catalogRouter.patch("/products/:id", adminOnly, asyncHandler(async (req, res) => {
...
catalogRouter.delete("/products/:id", adminOnly, asyncHandler(async (req, res) => {
  await svc.deleteProduct(req.params.id);
  res.status(204).send();
}));

catalogRouter.get("/materials/:id", canReadCatalog, asyncHandler(async (req, res) => {
  const material = await svc.getMaterial(req.params.id);
  if (!material) return res.status(404).json({ error: "Material not found" });
  res.json(material);
}));

catalogRouter.get("/materials", canReadCatalog, asyncHandler(async (_req, res) => {
  res.json(await svc.getMaterials());
}));

catalogRouter.post("/materials", adminOnly, asyncHandler(async (req, res) => {
  const data = insertMaterialSchema.parse(req.body);
  res.status(201).json(await svc.createMaterial(data));
}));

catalogRouter.patch("/materials/:id", adminOnly, asyncHandler(async (req, res) => {
...
catalogRouter.delete("/materials/:id", adminOnly, asyncHandler(async (req, res) => {
  await svc.deleteMaterial(req.params.id);
  res.status(204).send();
}));

catalogRouter.get("/recipes/:id/items", canReadCatalog, asyncHandler(async (req, res) => {
  res.json(await svc.getRecipeItemsWithMaterials(req.params.id));
}));

catalogRouter.get("/recipes/:id", canReadCatalog, asyncHandler(async (req, res) => {
  const recipe = await svc.getRecipe(req.params.id);
  if (!recipe) return res.status(404).json({ error: "Recipe not found" });
  res.json(recipe);
}));

catalogRouter.get("/recipes", canReadCatalog, asyncHandler(async (_req, res) => {
  res.json(await svc.getRecipes());
}));

catalogRouter.post("/recipes", adminOnly, asyncHandler(async (req, res) => {
...
catalogRouter.get("/process-code-definitions", canReadCatalog, asyncHandler(async (_req, res) => {
