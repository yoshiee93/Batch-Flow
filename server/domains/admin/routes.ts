...
adminRouter.post("/admin/import", adminOnly, asyncHandler(async (req, res) => {
  ...
  await db.transaction(async (tx) => {
    // Full restore: wipe everything including users, then re-insert from backup.
    // All original user IDs are preserved so every FK reference stays valid.
    // The current session is invalidated — the user logs back in with backup credentials.
    ...
    for (const u of backupUsers) {
      ...
      await tx.execute(sql`
        ...
        VALUES (
          ${u.id as string}, ${u.username as string}, ${u.password as string},
